...
bool CAddonMgr::DisableAddon(const CStdString& ID, bool disable)
{
  if (m_database.DisableAddon(ID, disable))
  {
    std::map<std::string, bool>::iterator it = m_disabled.find(ID);
    if (it == m_disabled.end())
      m_disabled.insert(pair<std::string, bool>(ID, disable));
    else
      it->second = disable;
...
bool CAddonMgr::IsAddonDisabled(const CStdString &ID)
{
