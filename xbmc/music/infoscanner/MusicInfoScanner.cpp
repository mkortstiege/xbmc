  ...
  if (albumDownloadStatus == INFO_NOT_FOUND)
  {
    ...
    else
    {
      CActivityLog::GetInstance().Add(ActivityPtr(new CMediaLibraryActivity(
        MediaTypeAlbum, album.strPath, 24137,
        StringUtils::Format(g_localizeStrings.Get(24138).c_str(), MediaTypeAlbum, album.strAlbum.c_str()),
        CScraperUrl::GetThumbURL(album.thumbURL.GetFirstThumb()), CURL::GetRedacted(album.strPath), ActivityLevelWarning)));
    }
  }
...
loop:
  ...
  if (artistDownloadStatus == INFO_NOT_FOUND)
  {
    ...
    {
      CActivityLog::GetInstance().Add(ActivityPtr(new CMediaLibraryActivity(
        MediaTypeArtist, artist.strPath, 24137,
        StringUtils::Format(g_localizeStrings.Get(24138).c_str(), MediaTypeArtist, artist.strArtist.c_str()),
        CScraperUrl::GetThumbURL(artist.thumbURL.GetFirstThumb()), CURL::GetRedacted(artist.strPath), ActivityLevelWarning)));
    }
  }
