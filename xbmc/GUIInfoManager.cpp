...
{
  auto i = std::find_if(std::cbegin(container), std::cend(container), [&val](const infomap& m)
  {
    return m.str == val;
  });
...
int CGUIInfoManager::TranslateSingleString(const std::string &strCondition, bool &listItemDependent)
{
  ...
  else if (info.size() == 2)
  {
    ...
    {
      auto offset = atoi(cat.param().c_str());
      auto ret = TranslateListItem(prop);
      if (ret)
        listItemDependent = true;
      if (offset)
  ...
  else if (info.size() == 3 || info.size() == 4)
  {
    ...
    { 
      ...
      if (platform == "atv2") return SYSTEM_PLATFORM_DARWIN_ATV2;
...
std::string CGUIInfoManager::GetItemLabel(const CFileItem *item, int info, std::string *fallback)
{
  ...
  return KODI::GUIINFO::CFileItemInfoProvider::GetLabel(std::make_shared<CFileItem>(*item), info);
}
